...
/// @title zonulet Token Multi-Registry
/// @notice Maintains multiple token lists, curated by the DAO
contract ZonuletTokenRegistry is Ownable {
    ...
    function addTokens(uint256 _listId, address[] memory _tokens) public onlyOwner {
        require(_listId <= listCount, 'ZonuletTokenRegistry : INVALID_LIST');
        for (uint32 i = 0; i < _tokens.length; i++) {
            require(
                tcrs[_listId].status[_tokens[i]] != TokenStatus.ACTIVE,
                'ZonuletTokenRegistry : DUPLICATE_TOKEN'
            );
    ...
    function removeTokens(uint256 _listId, address[] memory _tokens) public onlyOwner {
        ...
        for (uint32 i = 0; i < _tokens.length; i++) {
            require(
                tcrs[_listId].status[_tokens[i]] == TokenStatus.ACTIVE,
                'ZonuletTokenRegistry : INACTIVE_TOKEN'
            );
    ...
    function getTokensRange(
        ...
        require(
            ...
            'ZonuletTokenRegistry : INVALID_RANGE'
        );
        require(_start <= _end, 'ZonuletTokenRegistry : INVALID_INVERTED_RANGE');
